...
replay.tracker.events`;
  ...
  describe('error handling', () => {
    it('should handle corrupted replay data gracefully', () => {
      const corruptedBuffer = Buffer.alloc(1024);

      // Write minimal MPQ structure
      corruptedBuffer.writeUInt32LE(0x1A51504D, 0);  // Magic
      corruptedBuffer.writeUInt32LE(0x20, 4);        // Header size
      corruptedBuffer.writeUInt32LE(0x400, 8);       // Archive size
      corruptedBuffer.writeUInt32LE(0x0100, 16);     // Hash table pos
      corruptedBuffer.writeUInt32LE(0x0200, 20);     // Block table pos
      corruptedBuffer.writeUInt32LE(0x0000, 24);     // Hash table size (0)
      corruptedBuffer.writeUInt32LE(0x0000, 28);     // Block table size (0)

      // This should not throw, but create a replay with default values
      expect(() => {
        const replay = SC2Replay.fromBuffer(corruptedBuffer, {
          decodeGameEvents: false,
          decodeMessageEvents: false,
          decodeTrackerEvents: false
        });
    ...
    it('should handle minimal replay data', () => {
      const minimalBuffer = Buffer.alloc(1024);

      // Write minimal MPQ structure
      minimalBuffer.writeUInt32LE(0x1A51504D, 0);  // Magic
      minimalBuffer.writeUInt32LE(0x20, 4);        // Header size
      minimalBuffer.writeUInt32LE(0x400, 8);       // Archive size
      minimalBuffer.writeUInt32LE(0x0100, 16);     // Hash table pos
      minimalBuffer.writeUInt32LE(0x0200, 20);     // Block table pos
      minimalBuffer.writeUInt32LE(0x0000, 24);     // Hash table size (0)
      minimalBuffer.writeUInt32LE(0x0000, 28);     // Block table size (0)

      const replay = SC2Replay.fromBuffer(minimalBuffer, {
        decodeGameEvents: false,
        decodeMessageEvents: false,
        decodeTrackerEvents: false
      });
